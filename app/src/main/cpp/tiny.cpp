...
Java_com_jacky_finalexam_Yolo_Init(JNIEnv *env, jobject obj, jstring param, jstring bin) {
...
JNIEXPORT jfloatArray JNICALL Java_com_jacky_finalexam_Yolo_Detect(JNIEnv* env, jobject thiz, jobject bitmap)
{
...
//    env->ReleaseStringUTFChars( imgPath, img_path);
   ...
   if (result != 0)
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov2TinyJni", "width %d", output_wsize);
    __android_log_print(ANDROID_LOG_DEBUG, "yolov2TinyJni", "height %d", output_hsize);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov2TinyJni", "这里可以运行到吗");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "yolov2TinyJni", "那这里呢");
    return jOutputData;
